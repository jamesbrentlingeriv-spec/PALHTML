...
export default function ContactOrders() {
  ...
  const inp = 'w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-gray-500'

  return (
    <div className="min-h-screen bg-gray-100 p-5">
      <BackButton />
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-400 p-6">
        <h1 className="text-2xl font-bold text-center text-black mb-6">Contact Lens Order Sheet</h1>

        {/* Add Form */}
        <div className="bg-gray-100 rounded-xl p-5 mb-6 border border-gray-300">
          <h2 className="text-lg font-bold text-black mb-4">Add New Order</h2>
          ...
          <button onClick={add} className="mt-4 px-6 py-2 bg-gray-700 text-white rounded-full font-bold hover:bg-gray-800 transition-colors">Add to Order List</button>
        </div>

        {/* Table */}
        <div className="bg-gray-50 rounded-xl p-5 border border-gray-300">
          <h2 className="text-lg font-bold text-black mb-4">Contacts To Be Ordered</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead><tr className="bg-gray-200 text-gray-800">
              ...
              <tbody>
                {patients.map(p => (
                  <tr key={p.id} className="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td className="p-2">{p.supply === 'Custom Box Count' ? `${p.boxes} Boxes (${p.perEye ? 'Per Eye' : 'Total'})` : <span className="bg-gray-200 text-gray-800 px-2 py-0.5 rounded-full font-bold">{p.supply}</span>}</td>
                    <td className="p-2">{p.hasIns ? <><span className="bg-gray-300 text-gray-800 px-1 rounded text-xs font-bold mr-1">INS</span>{p.insName}</> : 'No'}</td>
                    <td className="p-2">{p.pay === 'Pay When It Comes In' ? <span className="text-gray-900 font-bold">{p.pay}</span> : p.pay}</td>
                    <td className="p-2 text-gray-600 italic max-w-32 truncate">{p.notes || '-'}</td>
                    <td className="p-2">{p.dateAdded}</td>
                    <td className="p-2"><button onClick={() => remove(p.id)} className="bg-gray-600 text-white px-2 py-0.5 rounded text-xs font-bold hover:bg-gray-800">X</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <button onClick={exportExcel} className="flex-1 py-2 bg-gray-600 text-white rounded-full font-bold hover:bg-gray-800 text-sm">Export to Excel</button>
            <button onClick={() => window.print()} className="flex-1 py-2 bg-gray-500 text-white rounded-full font-bold hover:bg-gray-700 text-sm">Print List 🖨️</button>
            <button onClick={() => { if (confirm('Clear entire list?')) { setPatients([]); localStorage.removeItem(STORAGE_KEY) } }} className="flex-1 py-2 bg-gray-700 text-white rounded-full font-bold hover:bg-gray-900 text-sm">Reset List</button>
          </div>
        </div>
        <p className="text-center text-xs text-gray-500 mt-4">jamesbrentlinger2026™</p>
      </div>
    </div>
  )
}