    ...
    <div className={`flex flex-col bg-theme-card overflow-hidden ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-theme-card">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-20">
            ...
            const isSelected = selectedItemName === item.name || selectedItemName === `LENS: ${item.name}`;
            return (
              <button
                key={`${item.name}-${idx}`}
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <span className={`p-1 px-2 rounded border text-[9px] font-black uppercase transition-colors ${
                    isSelected ? "bg-green-500 text-white border-green-600" : "bg-theme-bg border-theme-border text-theme-text"
                  }`}>
                    {item.category}
                  </span>
                  {(isSelected || isCommercial) && (
                    <Check className={`w-3 h-3 transition-opacity ${isSelected ? "text-green-600 opacity-100" : "text-theme-accent opacity-0 group-hover:opacity-100"}`} />
                  )}
                </div>
                <h4 className={`text-xs font-black leading-tight mb-2 line-clamp-2 uppercase italic transition-colors ${isSelected ? "text-green-700" : "text-theme-text"}`}>
                  {item.name}
                </h4>
                <div className="flex items-baseline gap-1">
                  <Tag className={`w-3 h-3 transition-colors ${isSelected ? "text-green-600" : "text-theme-text"}`} />
                  <span className={`text-[11px] font-black transition-colors ${isSelected ? "text-green-600" : isCommercial ? 'text-theme-accent' : 'text-theme-text'}`}>
