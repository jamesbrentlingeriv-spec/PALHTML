...
// Safe phone number formatter - prevents crashes if phone is undefined or short
const formatPhone = (phone: string = "") => {
  if (!phone || phone.length < 3) return "";
  return `( ${phone.slice(0, 3)} ) ${phone.slice(3)}`;
};

// Safe charge splitter - prevents crashes if charge is undefined or malformed
...
// Field component defined outside the main component to fix eslint "component created during render" error
function Field({
  top,
  left,
  children,
...
export default function CMSPrintView({ data }: PrintViewProps) {
  ...
  return (
    ...
    >
      ...
      {/* 1a. Insured's ID */}
      <Field top={0.42} left={5.75} width={2.5}>
        {data.insuredId}
      </Field>

      {/* 2. Patient's Name */}
      <Field top={0.78} left={0.25} width={3.3}>
        {data.patientName}
      </Field>

      {/* 3. Patient Birth Date */}
      <Field top={0.78} left={4.48}>
        {getMonth(data.patientBirthDate)}
      </Field>
      <Field top={0.78} left={4.82}>
        {getDay(data.patientBirthDate)}
      </Field>
      <Field top={0.78} left={5.15}>
        {getYear(data.patientBirthDate)}
      </Field>

      {/* 3. Sex */}
      {data.patientSex === "M" && (
        <Field top={0.92} left={5.45}>
          X
        </Field>
      )}
      {data.patientSex === "F" && (
        <Field top={0.92} left={6.03}>
          X
        </Field>
      )}
      ...
      {/* 5. Patient's Address */}
      <Field top={1.22} left={0.15}>
        {data.patientAddress.street}
      </Field>
      <Field top={1.53} left={0.15}>
        {data.patientAddress.city}
      </Field>
      <Field top={1.53} left={3.05}>
        {data.patientAddress.state}
      </Field>
      <Field top={1.84} left={0.15}>
        {data.patientAddress.zipCode}
      </Field>
      <Field top={1.84} left={1.9}>
        {formatPhone(data.patientAddress.phone)}
      </Field>

      {/* 6. Relationship */}
      {data.patientRelationship === "SELF" && (
        <Field top={1.22} left={4.45}>
          X
        </Field>
      )}
      {data.patientRelationship === "SPOUSE" && (
        <Field top={1.22} left={4.9}>
          X
        </Field>
      )}
      {data.patientRelationship === "CHILD" && (
        <Field top={1.22} left={5.46}>
          X
        </Field>
      )}
      {data.patientRelationship === "OTHER" && (
        <Field top={1.22} left={6.03}>
          X
        </Field>
      )}
      ...
      {data.patientRelationship !== "SELF" && (
        <>
          <Field top={1.22} left={6.5}>
            {data.insuredAddress.street}
          </Field>
          <Field top={1.53} left={6.5}>
            {data.insuredAddress.city}
          </Field>
          <Field top={1.53} left={7.8}>
            {data.insuredAddress.state}
          </Field>
          <Field top={1.84} left={6.5}>
            {data.insuredAddress.zipCode}
          </Field>
        </>
      )}
      ...
      {data.conditionRelatedTo.employment && (
        <Field top={2.15} left={4.46}>
          X
        </Field>
      )}
      {!data.conditionRelatedTo.employment && (
        <Field top={2.15} left={5.04}>
          X
        </Field>
      )}

      {data.conditionRelatedTo.autoAccident && (
        <Field top={2.46} left={4.46}>
          X
        </Field>
      )}
      {!data.conditionRelatedTo.autoAccident && (
        <Field top={2.46} left={5.04}>
          X
        </Field>
      )}
      {data.conditionRelatedTo.autoAccident && (
        <Field top={2.46} left={5.46}>
          {data.conditionRelatedTo.place}
        </Field>
      )}

      {data.conditionRelatedTo.otherAccident && (
        <Field top={2.77} left={4.46}>
          X
        </Field>
      )}
      {!data.conditionRelatedTo.otherAccident && (
        <Field top={2.77} left={5.04}>
          X
        </Field>
      )}

      {/* 11. Insured's Policy/Group */}
      <Field top={2.15} left={6.5}>
        {data.insuredGroupNumber}
      </Field>
      <Field top={2.77} left={6.5}>
        {data.insuredPlanName}
      </Field>

      {/* 12. Patient's Signature */}
      <Field top={3.45} left={1.2}>
        SIGNATURE ON FILE
      </Field>
      <Field top={3.45} left={3.8}>
        {formatDate(data.patientSignatureDate)}
      </Field>

      {/* 13. Insured's Signature */}
      {data.insuredSignatureAuthorized && (
        <Field top={3.45} left={6.8}>
          SIGNATURE ON FILE
        </Field>
      )}

      {/* 14. Date of Illness */}
      <Field top={4.1} left={0.15}>
        {getMonth(data.dateOfIllness)} {getDay(data.dateOfIllness)}{" "}
        {getYear(data.dateOfIllness)}
      </Field>
      <Field top={4.1} left={1.3}>
        {data.dateOfIllnessQual}
      </Field>
      ...
      <Field top={4.4} left={5.6} width={1}>
        {data.referringProviderNpi}
      </Field>

      {/* 21. Diagnosis Codes (Box 21) */}
      <Field top={5.0} left={6.2}>
        {data.diagnosisCodeIcd}
      </Field>
      {data.diagnosisCodes.slice(0, 4).map((code, idx) => (
        <Field
          key={idx}
          top={5.2 + (idx > 1 ? 0.3 : 0)}
          left={0.3 + (idx % 2 === 1 ? 1.5 : 0)}
        >
          {code}
        </Field>
      ))}
      ...
      {data.serviceLines.slice(0, 6).map((line, idx) => {
        ...
        return (
          <React.Fragment key={idx}>
            <Field top={rowTop} left={0.15}>
              {line.fromDate}
            </Field>
            <Field top={rowTop} left={1.15}>
              {line.toDate}
            </Field>
            <Field top={rowTop} left={2.05}>
              {line.placeOfService}
            </Field>
            <Field top={rowTop} left={2.35}>
              {line.emergency ? "X" : ""}
            </Field>
            <Field top={rowTop} left={2.65}>
              {line.cptCode}
            </Field>
            <Field top={rowTop} left={3.55}>
              {line.modifier}
            </Field>
            <Field top={rowTop} left={4.0}>
              {line.diagnosisPointer}
            </Field>
            <Field top={rowTop} left={4.5} width={0.7} align="right">
              {charge.dollars}
            </Field>
            <Field top={rowTop} left={5.22} width={0.2}>
              {charge.cents}
            </Field>
            <Field top={rowTop} left={5.45} width={0.3} align="center">
              {line.daysOrUnits}
            </Field>
            <Field top={rowTop} left={7.35} width={1.0}>
      ...
      {/* 25. Federal Tax ID */}
      <Field top={8.55} left={0.15}>
        {data.taxId}
      </Field>
      {data.taxIdType === "SSN" && (
        <Field top={8.55} left={2.35}>
          X
        </Field>
      )}
      {data.taxIdType === "EIN" && (
        <Field top={8.55} left={2.65}>
          X
        </Field>
      )}

      {/* 26. Patient Account No */}
      <Field top={8.55} left={3.0}>
        {data.patientAccountNo}
      </Field>

      {/* 27. Accept Assignment */}
      {data.acceptAssignment && (
        <Field top={8.55} left={4.7}>
          X
        </Field>
      )}
      {!data.acceptAssignment && (
        <Field top={8.55} left={5.3}>
          X
        </Field>
      )}
      ...
      {(() => {
        ...
        return (
          <>
            <Field top={8.55} left={5.8} width={0.8} align="right">
              {tc.dollars}
            </Field>
            <Field top={8.55} left={6.65}>
              {tc.cents}
            </Field>
          </>
        );
      ...
      {(() => {
        ...
        return (
          <>
            <Field top={8.55} left={7.0} width={0.6} align="right">
              {ap.dollars}
            </Field>
            <Field top={8.55} left={7.65}>
              {ap.cents}
            </Field>
          </>
        );
      ...
      <Field top={9.1} left={0.15}>
        SIGNATURE ON FILE
      </Field>
      <Field top={9.1} left={2.8}>
      ...
      <Field top={9.6} left={4.5}>
        {data.facilityInfo.npi}
      </Field>
      ...
      <Field top={9.6} left={7.5}>
        {data.billingProviderInfo.npi}
      </Field>
      <Field top={9.6} left={6.5}>
